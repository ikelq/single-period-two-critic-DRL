...
class SACAgent:
    ...
    def update_model(self) -> Tuple[torch.Tensor, ...]:
        ...
        qv_target = 50*reward[:, 1].reshape(-1, 1) + self.gamma * mask * (qv_pred_next - 0.5*alpha * log_prob)
        ...
        if self.total_step % self.policy_update_freq == 0:
            ...
            advantage = torch.min(self.qf_1(state, self.actor(state)[0]), self.qf_2(state, self.actor(state)[0]))\
                        + torch.min(self.qv_1(state, self.actor(state)[0]), self.qv_2(state, self.actor(state)[0]))
            # advantage = q_pred
